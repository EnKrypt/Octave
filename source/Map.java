...
public class Map extends JPanel{
	...
	**/
    ...
    public void loadPlayground(){
    	...
    	int s=13;
    	int wh=18;
    	add(new Box("bluebox.png",240-18,0));
		add(new Box("greenbox.png",206,200));
    	for(int i=0;i<s;++i){
    		add(new Box("bluebox.png",240-wh-i*16,i*8));
    		add(new Box("whitebox.png",240-wh+i*16,i*8));
    	}
    	for(int i=1;i<s;++i){
    		add(new Box("redbox.png",i*16-3,8*s+i*8-8));
    		add(new Box("greenbox.png",480-i*16-65,8*s+i*8-8));
    	}
    	add(new Block(new Sprite("tree1.png"),new Mask("tree1_mask.png"),21,55),480/2-61,270/2-81);
    	add(new Block(new Sprite("rock1.png"),new Mask("rock1_mask.png"),0,14),240-33,16);
	...
	**/
	public void step(){
		...
		for(int i=0;i<entities.size();i++){
			o1=entities.get(i);
			o1.step();
			if(o1 instanceof Collidable){
				//Check for collisions
				for(int j=i+1;j<entities.size();j++){
					o2=entities.get(j);
					if(o2 instanceof Collidable){
						if(CollisionEngine.pixel_collision((Collidable)o1,(Collidable)o2)){
							((Collidable)o1).collide((Collidable)o2);
							((Collidable)o2).collide((Collidable)o1);
						}
					}
				}
			}
